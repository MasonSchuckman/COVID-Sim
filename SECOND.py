...
# Constants
population_size = 400   # Total population size
...
social_distancing_radius = 1.2  # Radius within which individuals try to maintain distance
repulsive_force_strength = 0.00  # Strength of the repulsive force
...
immunity_period = 30    # Days recovered individuals remain immune before possibly becoming susceptible again
...
scat = ax.scatter([], [], s=15, edgecolors='none', facecolors='none')
...
def update_seirs_social_distancing(frame):
    ...
    susceptible_count = np.count_nonzero(status == SUSCEPTIBLE)
    exposed_count = np.count_nonzero(status == EXPOSED)
    infected_count = np.count_nonzero(status == INFECTED)
    recovered_count = np.count_nonzero(status == RECOVERED)
                        ...
                        for j in grid[ny * grid_width + nx]:
                            if status[j] == SUSCEPTIBLE and j not in new_exposed:
                                ...
                                if distance < R and np.random.random() < P:
                                    ...
                                    circle = Circle(positions[i], R/2, color='white', fill=False, linestyle='--')
                                    ax.add_patch(circle)
    ...
    # Update plot
    colors = ['cyan' if s == SUSCEPTIBLE else 'orange' if s == EXPOSED else 'red' if s == INFECTED else 'green' for s in status]
    ...
    # Update history
    t=frame
    history[t, SUSCEPTIBLE] = np.count_nonzero(status == SUSCEPTIBLE)
    history[t, EXPOSED] = np.count_nonzero(status == EXPOSED)
    history[t, INFECTED] = np.count_nonzero(status == INFECTED)
    history[t, RECOVERED] = np.count_nonzero(status == RECOVERED)

    ax2.clear()
    ax2.stackplot(range(steps), history[:, SUSCEPTIBLE], history[:, EXPOSED], history[:, INFECTED], history[:, RECOVERED],
             labels=['Susceptible', 'Exposed', 'Infected', 'Recovered'],
             colors=['cyan', 'orange', 'red', 'green'])
    ax2.legend(loc='upper right')
    ax2.set_title('SEIRS Model Simulation Over Time')
